...
with open('/home/timotheerabot/Documents/stage_LBBE/tab_reactions.txt') as document: #Conversion du document correspondances des ID réaction et kegg de métabolites de l'équation de réaction en variable "document", entrer le chemin du document dans le terminal

    dico_correspondance = {}

    for line in document :

        (nom, ID_enzyme,ID_species,ID_compound,ID_EC  ) = line.split(";")      
        ID_EC = ID_EC.strip("\n")
        ID_EC.strip(" ")
        if ID_EC != "nan" and ID_EC != "": 
            try :
                r = brenda.reactions.get_by_id(ID_EC)
                ID_species = "Escherichia coli"
